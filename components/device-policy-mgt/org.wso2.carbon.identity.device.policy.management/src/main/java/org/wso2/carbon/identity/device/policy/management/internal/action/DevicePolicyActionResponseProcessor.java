...
/**
 * Processes the response from the DEVICE_POLICY action (MDM compliance check).
 * A SUCCESS response from the MDM endpoint indicates the device is enrolled and compliant.
 * FAILED and ERROR responses use the default implementations from the parent interface.
 */
public class DevicePolicyActionResponseProcessor implements ActionExecutionResponseProcessor {
    ...
    public ActionExecutionStatus<Success> processSuccessResponse(
        ...
        return new SuccessStatus.Builder()
                .setResponseContext(flowContext.getContextData())
                .build();
    }
}
