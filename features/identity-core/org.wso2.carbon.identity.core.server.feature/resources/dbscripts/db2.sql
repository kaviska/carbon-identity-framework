...
/
CREATE TRIGGER IDN_DEVICE_POLICY_TRIG NO CASCADE BEFORE INSERT ON IDN_DEVICE_POLICY
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL
    BEGIN ATOMIC
        SET (NEW.ID) = (NEXTVAL FOR IDN_DEVICE_POLICY_SEQ);
    END
/
...
/
CREATE TRIGGER IDN_VC_TEMPLATE_CURSOR_KEY_TRIG NO CASCADE
    BEFORE INSERT
    ON IDN_VC_TEMPLATE
    REFERENCING NEW AS NEW
    FOR EACH ROW MODE DB2SQL
...
/
CREATE TRIGGER IDN_VC_CLAIMS_TRIG NO CASCADE
    BEFORE INSERT
    ON IDN_VC_CLAIMS
    REFERENCING NEW AS NEW
    FOR EACH ROW MODE DB2SQL
